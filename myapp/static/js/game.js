...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function genQuestions(){
    ...
    for(let i=0;i<totalQ;i++){
      ...
      qs.push({question,answer:ans,options:Array.from(opts).sort(()=>Math.random()-0.5)});
    }
    return qs;
  }
  ...
  function handleAnswer(card,picked,correct){
    ...
    if(picked===correct){
      ...
      answerCards.find(c=>+c.textContent===correct).classList.add('correct');
      delay=2500;
    }
    setTimeout(()=>{
      currentQ++;
